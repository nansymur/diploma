...
gtrends_orth19_wrist<-gtrends(
keyword = c("wrist brace"),
geo = "US",
...
``` {r}
data_orth19_wrist<-gtrends_orth19_wrist$interest_by_region%>%
...
``` {r}
data19_wrist<-data_orth19_wrist %>% mutate(year = "2019") %>%
...
```{r}
data_orth18_wrist<-gtrends_orth18_wrist$interest_by_region%>%
  replace_na(list(hits=0))%>%
  dplyr::select(-geo,-gprop,-keyword)%>%
  dplyr::rename("NAME"="location") %>%
  left_join(state_info)
...
``` {r}
data18_wrist<- data_orth18_wrist %>% dplyr::mutate(year = "2018") %>%
...
```{r}
data_orth17_wrist<-gtrends_orth17_wrist$interest_by_region%>%
  replace_na(list(hits=0))%>%
  dplyr::select(-geo,-gprop,-keyword)%>%
  dplyr::rename("NAME"="location") %>%
  left_join(state_info)
...
``` {r}
data17_wrist<- data_orth17_wrist %>% mutate(year = "2017") %>%
...
```{r}
data_orth16_wrist<-gtrends_orth16_wrist$interest_by_region%>%
  replace_na(list(hits=0))%>%
  dplyr::select(-geo,-gprop,-keyword)%>%
  dplyr::rename("NAME"="location") %>%
  left_join(state_info)
...
``` {r}
data16_wrist<- data_orth16_wrist %>% mutate(year = "2016") %>%
...
```{r}
data_orth15_wrist<-gtrends_orth15_wrist$interest_by_region%>%
  replace_na(list(hits=0))%>%
  dplyr::select(-geo, -gprop,-keyword)%>%
  dplyr::rename("NAME"="location") %>%
  left_join(state_info)
...
``` {r}
data15_wrist<- data_orth15_wrist %>% mutate(year = "2015") %>%
...
```{r} 
data_wrist = rbind(data19_wrist, data18_wrist, data17_wrist, data16_wrist, data15_wrist)
data_wrist = data_wrist %>% dplyr::select(-geometry, -GEOID)
#write.csv(data_wrist, "data_wrist.csv", row.names = F)
```
...
``` {r}
colnames(data_wrist)<-ifelse(substr(colnames(data_wrist),nchar(colnames(data_wrist)),
                              nchar(colnames(data_wrist)))=="E"&
                         !colnames(data_wrist) %in% c("NAME","latitude","longitude"), 
                       substr(colnames(data_wrist),1,nchar(colnames(data_wrist))-1),
                       colnames(data_wrist))
```

```{r}
data_wrist = data_wrist %>% mutate(perc_fem_20_21 = perc_fem_20 + perc_fem_21) 
data_wrist = data_wrist %>% dplyr::select(-perc_fem_21, -perc_fem_20)
data_wrist = data_wrist %>% mutate(perc_mal_20_21 = perc_mal_20 + perc_mal_21) 
data_wrist = data_wrist %>% dplyr::select(-perc_mal_21, -perc_mal_20)

data_wrist = data_wrist %>% mutate(perc_fem_0_9 = perc_fem_0_5 + perc_fem_5_9)
data_wrist = data_wrist %>% mutate(perc_fem_10_17 = perc_fem_10_14 + perc_fem_15_17)
data_wrist = data_wrist %>% mutate(perc_fem_18_21 = perc_fem_18_19 + perc_fem_20_21)
data_wrist = data_wrist %>% mutate(perc_fem_22_29 = perc_fem_22_24 + perc_fem_25_29)
data_wrist = data_wrist %>% mutate(perc_fem_30_39 = perc_fem_30_34 + perc_fem_35_39)
data_wrist = data_wrist %>% mutate(perc_fem_40_49 = perc_fem_40_44 + perc_fem_45_49)
data_wrist = data_wrist %>% mutate(perc_fem_50_59 = perc_fem_50_54 + perc_fem_55_59)
data_wrist = data_wrist %>% mutate(perc_fem_60_66 = perc_fem_60_61 + perc_fem_62_64 + perc_fem_65_66)
data_wrist = data_wrist %>% mutate(perc_fem_67_79 = perc_fem_67_69 + perc_fem_70_74 + perc_fem_75_79)
data_wrist = data_wrist %>% mutate(perc_fem_80_85 = perc_fem_80_84 + perc_fem_85_over)

data_wrist = data_wrist %>% mutate(perc_mal_0_9 = perc_mal_0_4 + perc_mal_5_9)
data_wrist = data_wrist %>% mutate(perc_mal_10_17 = perc_mal_10_14 + perc_mal_15_17)
data_wrist = data_wrist %>% mutate(perc_mal_18_21 = perc_mal_18_19 + perc_mal_20_21)
data_wrist = data_wrist %>% mutate(perc_mal_22_29 = perc_mal_22_24 + perc_mal_25_29)
data_wrist = data_wrist %>% mutate(perc_mal_30_39 = perc_mal_30_34 + perc_mal_35_39)
data_wrist = data_wrist%>% mutate(perc_mal_40_49 = perc_mal_40_44 + perc_mal_45_49)
data_wrist = data_wrist %>% mutate(perc_mal_50_59 = perc_mal_50_54 + perc_mal_55_59)
data_wrist = data_wrist %>% mutate(perc_mal_60_66 = perc_mal_60_61 + perc_mal_62_64 + perc_mal_65_66)
data_wrist = data_wrist %>% mutate(perc_mal_67_79 = perc_mal_67_69 + perc_mal_70_74 + perc_mal_75_79)
data_wrist = data_wrist %>% mutate(perc_mal_80_85 = perc_mal_80_84 + perc_mal_85_over)

data_wrist = data_wrist %>% dplyr::select(-perc_fem_0_5, -perc_fem_5_9, -perc_fem_10_14, -perc_fem_15_17, -perc_fem_18_19, -perc_fem_20_21, -perc_fem_22_24, -perc_fem_25_29, -perc_fem_30_34, -perc_fem_35_39, -perc_fem_40_44, -perc_fem_45_49, -perc_fem_50_54, -perc_fem_55_59, -perc_fem_60_61, -perc_fem_62_64, -perc_fem_65_66, -perc_fem_67_69, -perc_fem_70_74, -perc_fem_75_79, -perc_fem_80_84, -perc_fem_85_over)

data_wrist = data_wrist %>% dplyr::select(-perc_mal_0_4, -perc_mal_5_9, -perc_mal_10_14, -perc_mal_15_17, -perc_mal_18_19, -perc_mal_20_21, -perc_mal_22_24, -perc_mal_25_29, -perc_mal_30_34, -perc_mal_35_39, -perc_mal_40_44, -perc_mal_45_49, -perc_mal_50_54, -perc_mal_55_59, -perc_mal_60_61, -perc_mal_62_64, -perc_mal_65_66, -perc_mal_67_69, -perc_mal_70_74, -perc_mal_75_79, -perc_mal_80_84, -perc_mal_85_over)
```

```{r}
data_wrist = data_wrist %>% mutate(perc_household_income_less_14 = perc_household_inc_less_10 + perc_household_inc_10_14)
data_wrist = data_wrist %>% mutate(perc_household_income_15_24 = perc_household_inc_15_19 + perc_household_inc_20_24)
data_wrist = data_wrist %>% mutate(perc_household_income_25_34 = perc_household_inc_25_29 + perc_household_inc_30_34)
data_wrist = data_wrist %>% mutate(perc_household_income_35_44 = perc_household_inc_35_39 + perc_household_inc_40_44)
data_wrist = data_wrist %>% mutate(perc_household_income_45_59 = perc_household_inc_45_49 + perc_household_inc_50_59)   
data_wrist = data_wrist %>% mutate(perc_household_income_60_99 = perc_household_inc_60_74 + perc_household_inc_75_99)   
data_wrist = data_wrist %>% mutate(perc_household_income_100_149 = perc_household_inc_100_124 + perc_household_inc_125_149)  
data_wrist = data_wrist %>% mutate(perc_household_income_150_more = perc_household_inc_150_199 + perc_household_inc_200_more)   
 
data_wrist = data_wrist %>% dplyr::select(-perc_household_inc_less_10, -perc_household_inc_10_14, -perc_household_inc_15_19, -perc_household_inc_20_24, -perc_household_inc_25_29, -perc_household_inc_30_34, -perc_household_inc_35_39, -perc_household_inc_40_44, -perc_household_inc_45_49, -perc_household_inc_50_59, -perc_household_inc_60_74, -perc_household_inc_75_99, -perc_household_inc_100_124, -perc_household_inc_125_149, -perc_household_inc_150_199, -perc_household_inc_200_more) 
```
...
------------------
```{r} 
data_orth15_wrist %>% dplyr::select(NAME, hits) %>% head()
data_orth16_wrist %>% dplyr::select(NAME, hits) %>% head()
data_orth17_wrist %>% dplyr::select(NAME, hits) %>% head()
data_orth18_wrist %>% dplyr::select(NAME, hits) %>% head()
data_orth19_wrist %>% dplyr::select(NAME, hits) %>% head()
```
```{r} 
...
``` {r}
correl_wrist<-as.data.frame(cor(select_if(as.data.frame(data_wrist),is.numeric), method = "spearman"))
 
correl_wrist$variable=row.names(correl_wrist)

ggplot(subset(correl_wrist,!variable=="hits"),
              aes(y=reorder(variable, hits),x=hits))+
         geom_col()+
         theme_minimal()+
  labs(y="Variable",x="Correlation with interest in wrist brace")
```

Lowest smoothing of pairwise relationships

``` {r}  
ggplot(data=data_wrist,aes(x=perc_mal_60_66,y=hits,label=state_code))+
...
```{r}
data1_wrist = data_wrist %>% dplyr::select(-state_code, -perc_never_marr, -perc_mal_0_9, -perc_mal_10_17, -perc_mal_18_21, -perc_mal_22_29)
...
library(moments)
skewness(data1_wrist$hits)
kurtosis(data1_wrist$hits)
# normal kurtosis = 3, skewness = 0

qqnorm(data1_wrist$hits, pch = 1, frame = FALSE, main = "Normal Q-Q plot for GSVI of wrist brace")
qqline(data1_wrist$hits, col = "steelblue", lwd = 2)
 
```
```{r}
outliers <- boxplot(data1_wrist$hits, plot=FALSE)$out
d_wrist<-data1_wrist
d_wrist<- d_wrist[-which(d_wrist$hits %in% outliers),]
```

# After deleting outliers
```{r}
library(moments)
skewness(d_wrist$hits)
kurtosis(d_wrist$hits)
# normal kurtosis = 3, skewness = 0

qqnorm(d_wrist$hits, pch = 1, frame = FALSE, main = "Normal Q-Q plot for GSVI of wrist brace")
qqline(d_wrist$hits, col = "steelblue", lwd = 2)
```
# heterogeneity test
```{r}  
leveneTest(hits ~ NAME, data = data1_wrist)
fligner.test(hits ~ NAME, data = data1_wrist)
```
...
```{r} 
data_without = data1_wrist %>% dplyr::select(-NAME, -year)
ols_wrist <- lm(hits~., data_without)
summary(ols_wrist)
alias(ols_wrist)

yhat <- ols_wrist$fitted
plot(data1_wrist$hits, yhat)
```
...
```{r}
pcdtest(fixed_wrist, test = c("lm")) # cross-sectional dependence exists
...
```{r} 
pFtest(fixed_wrist, ols_wrist) # we need to use fixed effects 
```

```{r}  
random_wrist <- plm(f, data=data1_wrist, index=c("NAME", "year"), model="random")
summary(random_wrist)
```

Check if random or fixed should be used
```{r}
phtest(fixed_wrist, random_wrist)
# random effects should be used
```
...
fixed.time_wrist <- plm(ye, 
                 data = data1_wrist,  
                 index = c("NAME", "year"), 
                 model = "within")
summary(fixed.time_wrist)
pFtest(fixed.time_wrist, fixed_wrist)  

plmtest(fixed_wrist, c("time"), type=("bp"))
...
predictInterval(mult_wrist)   # for various model predictions, possibly with new data

REsim(mult_wrist)             # mean, median and sd of the random effect estimates

plotREsim(REsim(mult_wrist)) + labs(title = "Plot of Random Effects for wrist brace") 
```

## transforming into DUMMY -> states and year
```{r}
binary_state <- to.dummy(data1_wrist$NAME, "state") # convert character variables into dummy
binary_state = as.data.frame(binary_state) 
binary_state = binary_state %>% mutate(NAME = data1_wrist$NAME)
 
pen_wrist = data1_wrist %>% inner_join(binary_state) %>% unique() 

binary_year <- to.dummy(data1_wrist$year, "year") # convert character variables into dummy
binary_year = as.data.frame(binary_year) 
binary_year = binary_year %>% mutate(year = data1_wrist$year)
 
pen_wrist = pen_wrist %>% inner_join(binary_year) %>% unique() 

pen_wrist = pen_wrist %>% dplyr::select(-year, -NAME)
# Classes
length(grep("state.", names(pen_wrist), value=TRUE)) #dummy 
state = grep("state.", names(pen_wrist), value=TRUE)
year = grep("year.", names(pen_wrist), value=TRUE)
```
...
```{r} 
model2 <- train(hits ~., data = pen_wrist, preprocess = c("center", "scale"), method = "lm", trControl = ctrlspecs_lm, na.action=na.omit)
```
# Variable importance
...
```{r}
predictions2 <- predict(model2, newdata = pen_wrist)
```

# model performance/accuracy
```{r}
library(MLmetrics)
Modelperf2 <- data.frame(MSE = MSE(predictions2, pen_wrist$hits), RMSE = RMSE(predictions2, pen_wrist$hits), Rsq = R2(predictions2, pen_wrist$hits))
Modelperf2
```
...
```{r}  
data1 = as.data.frame(pen_wrist)

x = model.matrix(hits~., pen_wrist)[,-1] # trim off the first column
...
ridge_4 <- data.frame(regr = "ridge_l4", MSE = MSE(ridge_pred4, pen_wrist$hits), RMSE = RMSE(ridge_pred4, pen_wrist$hits), Rsq = R2(ridge_pred4, pen_wrist$hits), mean_coeff = mean(predict(ridge_mod, s = 4, type = "coefficients")))
ridge_4
```
...
ridge_1010 <- data.frame(regr = "ridge_l1010", MSE = MSE(ridge_pred_large, pen_wrist$hits), RMSE = RMSE(ridge_pred_large, pen_wrist$hits), Rsq = R2(ridge_pred_large, pen_wrist$hits), mean_coeff = mean(predict(ridge_mod, s = 1e10, type="coefficients")))
ridge_1010
```
...
ridge_0 <- data.frame(regr = "ridge_l0", MSE = MSE(ridge_pred0, pen_wrist$hits), RMSE = RMSE(ridge_pred0, pen_wrist$hits), Rsq = R2(ridge_pred0, pen_wrist$hits), mean_coeff = mean(predict(ridge_mod, s = 0, type="coefficients")))
ridge_0
```
...
```{r}
ridge_bestlamb <- data.frame(regr = "ridge_cv", MSE = MSE(ridge_pred_cv, pen_wrist$hits), RMSE = RMSE(ridge_pred_cv, pen_wrist$hits), Rsq = R2(ridge_pred_cv, pen_wrist$hits), mean_coeff = mean(predict(ridge_mod, type = "coefficients", s = bestlam))) # Display coefficients using lambda chosen by CV
ridge_bestlamb
```
...
```{r}
model0 <- train(hits ~ ., data = pen_wrist, preprocess = c("center", "scale"), method = "glmnet", 
...
# Model prediction
```{r}
predictions0 <- predict(model0, newdata = pen_wrist)
```

# model performance/accuracy
```{r}
library(MLmetrics)
Modelperf0 <- data.frame(MSE = MSE(predictions0, pen_wrist$hits), RMSE = RMSE(predictions0, pen_wrist$hits), Rsq = R2(predictions0, pen_wrist$hits))
Modelperf0
```
...
lasso_05 <- data.frame(regr = "lasso_l0.5", MSE = MSE(lasso_pred_05, pen_wrist$hits), RMSE = RMSE(lasso_pred_05, pen_wrist$hits), Rsq = R2(lasso_pred_05, pen_wrist$hits), mean_coeff = mean(predict(lasso_mod, type = "coefficients", s = 0.5)))
lasso_05
```
...
```{r}
lasso_cv <- data.frame(regr = "cv", MSE = MSE(lasso_pred_cv, pen_wrist$hits), RMSE = RMSE(lasso_pred_cv, pen_wrist$hits), Rsq = R2(lasso_pred_cv, pen_wrist$hits), mean_coeff = mean(predict(lasso_mod, type = "coefficients", s = bestlam)))
lasso_cv
```
...
lasso_50 <- data.frame(regr = "lasso_l50", MSE = MSE(lasso_pred_50, pen_wrist$hits), RMSE = RMSE(lasso_pred_50, pen_wrist$hits), Rsq = R2(lasso_pred_50, pen_wrist$hits), mean_coeff = mean(predict(lasso_mod, type = "coefficients", s = 50))) 
lasso_50
```
...
# Specify lasso regr model to be estimated using training data and 10-fold cv framework
```{r} 
model1 <- train(hits ~ ., data = pen_wrist, preprocess = c("center", "scale"), method = "glmnet", 
...
# Model prediction
```{r}
predictions1 <- predict(model1, newdata = pen_wrist) 
```

# model performance/accuracy
```{r}
Modelperf1 <- data.frame(MSE = MSE(predictions1, pen_wrist$hits), RMSE = RMSE(predictions1, pen_wrist$hits), Rsq = R2(predictions1, pen_wrist$hits))
Modelperf1
```
